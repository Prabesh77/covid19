...
const GlobalStats = ({ allData }) => {
  ...
  return (
    <div>
      ...
      <h3 className="title" ref={(el) => (title = el)}>
        Live Record of Covid-19 Victims
      </h3>
      <Statcard className="stats-wrapper" style={{ marginTop: "2rem" }}>
        <div className="stat-card" ref={(el) => (statcard1 = el)}>
          <p>Total Confirmed Cases</p>{" "}
          <p>{numberWithCommas(allData.confirmed.value)}</p>
        </div>
        <div className="stat-card" ref={(el) => (statcard2 = el)}>
          <p>Total Recovered</p>{" "}
          <p> {numberWithCommas(allData.recovered.value)}</p>
        </div>
        <div className="stat-card" ref={(el) => (statcard3 = el)}>
